  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-semibold rounded-lg shadow-sm transition-all"
            >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-slate-800/60 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-lg border border-slate-700 transition-all"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Total Headcount */}
        <div className="bg-slate-800/60/50 p-5 rounded-xl border border-slate-700/50 shadow-xs hover:border-cyan-600/40 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Total Workforce</span>
            <div className="w-8 h-8 rounded-lg bg-cyan-950/40 flex items-center justify-center text-cyan-400">
              <Users className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-100">{stats?.totalEmployees || 0}</span>
            <span className="text-xs font-medium text-teal-400">Active Staff</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1">Across 4 departments</p>
        </div>

        {/* Attendance Rate */}
        <div className="bg-slate-800/60/50 p-5 rounded-xl border border-slate-700/50 shadow-xs hover:border-teal-600/40 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Present Today</span>
            <div className="w-8 h-8 rounded-lg bg-teal-950/40 flex items-center justify-center text-teal-400">
              <UserCheck className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-100">{stats?.presentToday || 0}</span>
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-teal-950/60 text-teal-300">
        ...
        {/* Pending Approvals */}
        <div className="bg-slate-800/60/50 p-5 rounded-xl border border-slate-700/50 shadow-xs hover:border-amber-700/50 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Pending Leave Requests</span>
            <div className="w-8 h-8 rounded-lg bg-amber-950/40 flex items-center justify-center text-amber-600">
              <CalendarOff className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-amber-600">{pendingLeaves.length}</span>
            <span className="text-xs font-medium text-amber-400">Requires Action</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-1">Awaiting manager approval</p>
        </div>

        {/* Monthly Payroll Total */}
        <div className="bg-slate-800/60/50 p-5 rounded-xl border border-slate-700/50 shadow-xs hover:border-purple-700/50 transition-all">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-500">Monthly Net Payroll</span>
            <div className="w-8 h-8 rounded-lg bg-purple-950/40 flex items-center justify-center text-purple-600">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-800/60/50 rounded-xl border border-slate-700/50 shadow-xs p-5 space-y-4 min-w-0">
          <div className="flex items-center justify-between">
            <div>
          ...
          {pendingLeaves.length === 0 ? (
            <div className="py-8 text-center bg-slate-900/80/40 rounded-xl border border-dashed border-slate-700/50">
              ...
              {pendingLeaves.map(leave => (
                <div key={leave.id} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-start gap-3">
                    <div className="w-9 h-9 rounded-full bg-indigo-100 text-cyan-300 flex items-center justify-center font-bold text-xs flex-shrink-0">
                      {leave.employeeName.charAt(0)}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-slate-100">{leave.employeeName}</span>
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-800/60/40 text-slate-400 font-medium">
                          {leave.department}
                        </span>
                      ...
                      <p className="text-xs text-slate-400 mt-1 line-clamp-1">{leave.reason}</p>
                      <p className="text-[11px] text-slate-400 mt-0.5">
                        Duration: <span className="font-semibold text-slate-300">{leave.startDate}</span> to{' '}
                        <span className="font-semibold text-slate-300">{leave.endDate}</span> ({leave.totalDays} days)
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-center">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-xs font-semibold flex items-center gap-1 transition-all disabled:opacity-50"
                    >
                    ...
                    <button
                      ...
                      disabled={reviewingId === leave.id}
                      className="px-3 py-1.5 rounded-lg bg-amber-950/40 hover:bg-rose-100 text-amber-300 text-xs font-semibold flex items-center gap-1 border border-rose-200 transition-all disabled:opacity-50"
                    >
        ...
        <div className="bg-slate-800/60/50 rounded-xl border border-slate-700/50 shadow-xs p-5 space-y-4 min-w-0">
          <div>
            <h2 className="text-sm font-bold text-slate-100 flex items-center gap-2">
            ...
            {Object.entries(departmentCounts).map(([dept, count]) => {
              ...
              return (
                <div key={dept} className="space-y-1">
                  ...
                  <div className="w-full h-2 rounded-full bg-slate-800/60/40 overflow-hidden">
                    <div
          ...
          <div className="pt-4 border-t border-slate-700/30">
            <h3 className="text-xs font-semibold text-slate-100 mb-2">Quick Navigation</h3>
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={() => setActiveTab('employees')}
                className="p-2.5 rounded-lg bg-slate-900/80/40 hover:bg-cyan-950/40 hover:text-cyan-300 border border-slate-700/50 text-left transition-all text-xs font-medium"
              >
                Employees Directory
              </button>
              <button
      ...
      <div className="bg-slate-800/60/50 rounded-xl border border-slate-700/50 shadow-xs p-5 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <button
            onClick={() => setActiveTab('attendance')}
            className="text-xs text-cyan-400 font-semibold hover:text-cyan-300 flex items-center gap-1 self-start sm:self-auto"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            <thead>
              <tr className="border-b border-slate-700/30 text-slate-400 font-semibold uppercase tracking-wider text-[10px]">
            ...
            <tbody className="divide-y divide-slate-700/30">
              {employees.map(emp => {
                ...
                return (
                  <tr key={emp.id} className="hover:bg-slate-900/80/40/80 transition-colors">
                    ...
                    <td className="py-3 text-slate-400">{emp.jobDetails.department}</td>
                    ...
                    <td className="py-3">
                      {att?.status === 'Present' ? (
                        <span className="px-2 py-0.5 rounded-full bg-teal-950/60 text-teal-300 text-[10px] font-semibold">
                          Present
                        </span>
                      ) : att?.status === 'Leave' ? (
                        <span className="px-2 py-0.5 rounded-full bg-amber-950/60 text-amber-300 text-[10px] font-semibold">
                          On Leave
                        </span>
                      ) : (
                    ...
                    <td className="py-3 text-right">
                      <button
                        ...
                        className="text-xs font-semibold text-cyan-400 hover:text-indigo-900 bg-cyan-950/40 hover:bg-indigo-100 px-2.5 py-1 rounded-md transition-colors"
                      >
                        Inspect Profile
                      </button>
                    </td>
