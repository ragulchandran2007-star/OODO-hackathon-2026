  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-2">
          {role === 'admin' && (
            <button
              ...
              className="flex items-center gap-2 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-all"
            >
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-xs rounded-lg border border-slate-200 focus:outline-indigo-600"
          />
        </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center bg-white rounded-xl border border-dashed border-slate-200 p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {employees.map(emp => (
            <div
              ...
              className="bg-white rounded-xl border border-slate-200 p-5 shadow-xs hover:border-indigo-300 hover:shadow-md transition-all cursor-pointer flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-indigo-600 font-medium">{emp.jobDetails.title}</p>
                      <div className="flex items-center gap-1.5 mt-1">
                        <span className="text-[10px] px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 font-semibold font-mono">
                          {emp.employeeId}
                        </span>
              ...
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                <div className="text-[11px] text-slate-500">
                  Net: <span className="font-semibold text-slate-900">${emp.salaryStructure.netSalary.toLocaleString()}</span>/mo
                </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {employees.map(emp => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3 text-slate-700 font-medium">{emp.jobDetails.department}</td>
                    <td className="px-5 py-3 text-slate-600">{emp.jobDetails.title}</td>
                    ...
                    <td className="px-5 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                        emp.role === 'admin' ? 'bg-amber-100 text-amber-800' : 'bg-indigo-100 text-indigo-800'
                      }`}>
                    ...
                    <td className="px-5 py-3 text-right">
                      <button
                        ...
                        className="text-xs font-semibold text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1 rounded-md"
                      >
                        Inspect
                      </button>
                    </td>
