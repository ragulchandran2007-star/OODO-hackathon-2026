...
export const ProjectExportModal: React.FC<ProjectExportModalProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs overflow-y-auto">
      <div className="bg-slate-800/60/50 rounded-2xl shadow-2xl border border-slate-700/50 w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 my-6">
        ...
        <div className="px-6 py-4 bg-slate-900/80 text-white flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-teal-500/20 text-emerald-300 font-semibold border border-emerald-500/30">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white text-xs font-bold rounded-lg shadow-sm transition-all cursor-pointer"
            >
        ...
        <div className="flex items-center gap-2 px-6 pt-3 border-b border-slate-700/50 bg-slate-900/80/40 text-xs font-semibold text-slate-400">
          <button
            ...
            className={`pb-3 px-3 border-b-2 flex items-center gap-1.5 transition-colors ${
              activeTab === 'flow'
                ? 'border-indigo-600 text-cyan-400 font-bold'
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          ...
          {activeTab === 'flow' && (
            <div className="space-y-6">
              <div>
                <h3 className="text-sm font-bold text-slate-100">End-to-End System Flow Architecture</h3>
              ...
              <div className="p-6 bg-slate-900/80 text-white rounded-2xl border border-slate-800 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  {/* Step 1 */}
                  <div className="p-4 rounded-xl bg-slate-800/60/80 border border-slate-700 space-y-2">
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-cyan-500/20 text-indigo-300 font-bold">
                      1. React Client (SPA)
                    </span>
                  ...
                  {/* Step 2 */}
                  <div className="p-4 rounded-xl bg-slate-800/60/80 border border-slate-700 space-y-2">
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-teal-500/20 text-emerald-300 font-bold">
                      2. API Client / Fetch
                    </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl border border-slate-700/50 bg-slate-900/80/40 space-y-2">
                  <h4 className="text-xs font-bold text-slate-100 flex items-center gap-1.5">
                    <CheckCircle2 className="w-4 h-4 text-cyan-400" /> Employee & Admin Role Matrix
                  </h4>
                ...
                <div className="p-4 rounded-xl border border-slate-700/50 bg-slate-900/80/40 space-y-2">
                  <h4 className="text-xs font-bold text-slate-100 flex items-center gap-1.5">
                    <CheckCircle2 className="w-4 h-4 text-teal-400" /> Dual-Mode Storage Architecture
                  </h4>
                  <ul className="text-xs text-slate-400 space-y-1 pl-5 list-disc">
                    <li>Cloud preview mode: In-memory seed database with zero-setup instant demo.</li>
                    <li>MongoDB production mode: Connects to MongoDB Atlas via <code className="text-cyan-400 font-mono">MONGODB_URI</code>.</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'structure' && (
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-bold text-slate-100">Complete Project Directory Tree</h3>
                <p className="text-xs text-slate-500">
    ...
    └── utils/
        └── generateZip.ts           # JSZip packaging engine`}</pre>
          ...
          {activeTab === 'mongodb' && (
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-bold text-slate-100">Mongoose / MongoDB Schema Declarations</h3>
                <p className="text-xs text-slate-500">
          ...
          {activeTab === 'instructions' && (
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-bold text-slate-100">How to Run Locally on Your Machine</h3>
                <p className="text-xs text-slate-500">
              ...
              <div className="space-y-3 text-xs">
                <div className="p-4 bg-slate-900/80/40 rounded-xl border border-slate-700/50 space-y-2">
                  ...
                  <div className="p-2.5 bg-slate-900/80 text-slate-200 rounded-lg font-mono">
                ...
                <div className="p-4 bg-slate-900/80/40 rounded-xl border border-slate-700/50 space-y-2">
                  ...
                  <p className="text-slate-400">
                    Open <code className="font-mono text-cyan-400">http://localhost:3000</code> in your browser!
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-3 bg-slate-900/80/40 border-t border-slate-700/50 flex items-center justify-between">
          ...
          <button
            onClick={handleDownload}
            disabled={downloading}
            className="flex items-center gap-1.5 px-5 py-2 bg-cyan-600 hover:bg-cyan-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-all"
          >
