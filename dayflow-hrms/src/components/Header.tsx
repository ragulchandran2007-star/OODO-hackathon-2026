...
export const Header: React.FC<HeaderProps> = ({
  ...
  const { user, role, logout, switchUser, setActiveTab } = useAuth();
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="hidden lg:flex items-center gap-1 bg-slate-100 p-1 rounded-lg border border-slate-200 text-xs">
            ...
            <button
              onClick={() => switchUser('sarah.hr@dayflow.io')}
              className={`px-2.5 py-1 rounded-md transition-all font-medium ${
                user?.email === 'sarah.hr@dayflow.io'
            ...
            <button
              onClick={() => switchUser('david.chen@dayflow.io')}
              className={`px-2.5 py-1 rounded-md transition-all font-medium ${
                user?.email === 'david.chen@dayflow.io'
                  ? 'bg-white text-indigo-700 shadow-xs font-semibold'
                  : 'text-slate-600 hover:text-slate-900'
              }`}
            >
            ...
            <button
              onClick={() => switchUser('elena.rostova@dayflow.io')}
              className={`px-2.5 py-1 rounded-md transition-all font-medium ${
                user?.email === 'elena.rostova@dayflow.io'
                  ? 'bg-white text-indigo-700 shadow-xs font-semibold'
                  : 'text-slate-600 hover:text-slate-900'
              }`}
            >
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold shadow-xs transition-all cursor-pointer"
            title="Download full full-stack source code with MongoDB Backend and React Frontend"
          >
            <Download className={`w-3.5 h-3.5 ${isExporting ? 'animate-bounce' : ''}`} />
            <span className="hidden sm:inline">Download Project ZIP</span>
          </button>

          {/* Project Architecture Flow Button */}
          <button
            onClick={onOpenProjectExport}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 text-xs font-medium transition-all"
            title="View Project Architecture & MongoDB Schema"
          >
            <FileCode className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Flow & DB</span>
          </button>
