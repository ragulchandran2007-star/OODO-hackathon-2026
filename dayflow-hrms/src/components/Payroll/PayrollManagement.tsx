        ...
        <div className="flex items-center gap-2">
          {isAdmin && (
            <button
              ...
              disabled={generating}
              className="flex items-center gap-2 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-sm transition-all disabled:opacity-50"
            >
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {payrollList.length > 0 && (
              <button
                ...
                className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold rounded-xl shadow-md flex items-center gap-2"
              >
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="text-xs p-2 rounded-lg border border-slate-300 bg-white focus:outline-indigo-600 font-medium"
          >
              ...
              {loading ? (
                ...
                payrollList.map(record => (
                  <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-slate-700 font-medium">${record.basicSalary.toLocaleString()}</td>
                    <td className="px-5 py-3 text-slate-700 font-medium">${(record.hra + record.allowances).toLocaleString()}</td>
                    <td className="px-5 py-3 font-semibold text-slate-900">${record.grossSalary.toLocaleString()}</td>
                    <td className="px-5 py-3 font-medium text-rose-600">-${record.totalDeductions.toLocaleString()}</td>
                    <td className="px-5 py-3 font-bold text-emerald-700 text-sm">
                      ${record.netSalary.toLocaleString()}
                    </td>
                    <td className="px-5 py-3">
                      <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-semibold flex items-center gap-1 w-fit">
                    ...
                    <td className="px-5 py-3 text-right">
                      <button
                        ...
                        className="px-3 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-semibold rounded-md transition-all flex items-center gap-1 ml-auto"
                      >
