    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
      <div className="bg-slate-800/60/50 rounded-2xl shadow-2xl border border-slate-700/50 w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="p-4 bg-slate-900/80/40 rounded-xl border border-slate-700/50 space-y-2 text-xs">
            <div className="flex items-center justify-between">
              <span className="font-bold text-sm text-slate-100">{leave.employeeName}</span>
              <span className="px-2 py-0.5 rounded-full bg-indigo-100 text-cyan-300 font-semibold">
            ...
            <div className="pt-2 border-t border-slate-700/50 flex justify-between font-medium text-slate-300">
              <span>Dates: {leave.startDate} to {leave.endDate}</span>
              <span className="text-cyan-400 font-bold">{leave.totalDays} day(s)</span>
            </div>
            <div className="pt-1 text-slate-400 bg-slate-800/60/50 p-2.5 rounded-lg border border-slate-700/50">
          ...
          {/* Decision Selector */}
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-2">Decision Action</label>
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`py-3 px-4 rounded-xl border text-xs font-bold flex items-center justify-center gap-2 transition-all ${
                  status === 'Approved'
                    ? 'bg-teal-950/40 border-emerald-500 text-teal-300 ring-2 ring-emerald-500/20'
                    : 'bg-slate-800/60/50 border-slate-700/50 text-slate-400 hover:bg-slate-900/80/40'
                }`}
              >
              ...
              <button
                ...
                className={`py-3 px-4 rounded-xl border text-xs font-bold flex items-center justify-center gap-2 transition-all ${
                  status === 'Rejected'
                    ? 'bg-amber-950/40 border-rose-500 text-amber-300 ring-2 ring-rose-500/20'
                    : 'bg-slate-800/60/50 border-slate-700/50 text-slate-400 hover:bg-slate-900/80/40'
                }`}
              >
          ...
          <div className="pt-3 border-t border-slate-700/30 flex items-center justify-end gap-2">
            ...
            <button
              ...
              className={`px-5 py-2 text-white text-xs font-bold rounded-lg shadow-sm transition-all flex items-center gap-1.5 ${
                status === 'Approved' ? 'bg-teal-600 hover:bg-teal-700' : 'bg-rose-600 hover:bg-rose-700'
              }`}
            >
